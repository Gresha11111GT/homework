...
public class Bank {
    ...
    public synchronized void deposit(String name, int moneyToAdd){
        BankAccount ba = getCardByName(name);
        if (ba!=null){
            if (ba.isStateOfAcc()){
                ba.addMoney(moneyToAdd);
                System.out.println("Бабла стало більше: " + ba.getName() + " на:" + ba.getMoney());
            }
            else
    ...
    public synchronized void withdraw(String name, int moneyToRemove){
        BankAccount ba = getCardByName(name);
        if (ba!=null){
            if (ba.isStateOfAcc()) {
                if (!(ba.getMoney() < moneyToRemove)){
                    ba.setMoney(ba.getMoney()-moneyToRemove);
                    System.out.println("Бабла стало менше: " + ba.getName() + " на:" + ba.getMoney());
                }
                else
